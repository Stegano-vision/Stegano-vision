...
def rbml_em(test_str):
  ...
  if detected_circles is not None:
      ...
      for pt in detected_circles[0, :]:
          ...
          # if(actual_question==0 and 0<=count<=160):
          #   actual_question=40
          # elif(actual_question==0 and 161<=count<=320):
          #   actual_question=80
          # elif(actual_question==0 and 321<=count<=480):
          #   actual_question=120
          # elif(actual_question==0 and 481<=count<=640):
          #   actual_question=160
          # elif(actual_question==0 and 641<=count<=800):
          #   actual_question=200
  ...
  #print(count)
  #status = cv2.imwrite("C:/Users/glair/Desktop/mo4_4.png", img)
  
  ts = time.time()
  ts = str(ts).split('.')[0]
  cv2.imwrite(r"C:\Users\glair\Desktop\S7 project\output\{}.png".format(ts), img)
  ###return(base64ToString(base64.b64encode(img)))
  return(ts)
